...
(function () {
  ...
  /* ==========================================================================
   * Relationship layer (roadmap v2 — the "world model").
   * Links live on `entity.links = { [relKey]: [entityId, ...] }` — pure data,
   * lossless in the World JSON, backward-compatible (missing => no links).
   * Reverse links are COMPUTED on the fly, never stored (no duplication, no
   * drift). Adding a new relation here is content-only.
   * ======================================================================== */
  const RELATIONS = {
    character: [
      {
        key: "settings", label: "Belongs to", target: "setting", reverseLabel: "Characters here",
        teach: `<p>Link this character to the <strong>settings they belong to</strong> — where they live, work, or hold power. This is what turns a stack of cards into a <em>world model</em>: the character and the place now cross-reference each other, and you can click straight between them.</p>`,
        example: "Kestrel Vance → The Ashfall Market (he works its debt every day).",
      },
    ],
    event: [
      {
        key: "characters", label: "Linked characters", target: "character", reverseLabel: "Appears in",
        teach: `<p>Your <em>Who's involved</em> field lists everyone loosely — including people who'll never get their own card. Here you <strong>link the ones that do</strong>, so the event and each character point at each other. Later you can click through to trace who was where.</p>`,
        example: "The Drowning of the North Fleet → Kestrel Vance.",
      },
      {
        key: "setting", label: "Takes place in", target: "setting", single: true, reverseLabel: "Events here",
        teach: `<p>Anchor the event to <strong>where it happened</strong>. Once linked, that setting's own card lists this event automatically — your locations start to carry their own history instead of being inert backdrops.</p>`,
        example: "The Drowning of the North Fleet → The Ashfall Market's harbor.",
      },
    ],
    setting: [],
  };

  function relationsFor(type) { return RELATIONS[type] || []; }

  // The wizard walks content steps (SCHEMA) followed by relation steps (links).
  function wizardSteps(type) {
    const schema = SCHEMAS[type];
    if (!schema) return [];
    const linkSteps = relationsFor(type).map((r) => ({
      key: r.key, label: r.label, type: "links", target: r.target, single: r.single,
      teach: r.teach, example: r.example, relation: r,
    }));
    return schema.steps.concat(linkSteps);
  }

  // Resolve a card's connections: forward links + computed reverse links.
  // Returns [{ label, items: [{ id, name, type }] }, ...].
  function resolveConnections(world, entity) {
    if (!world || !entity) return [];
    const byId = (id) => world.entities.find((e) => e.id === id);
    const groups = [];
    for (const rel of relationsFor(entity.type)) {
      const ids = (entity.links && entity.links[rel.key]) || [];
      const items = ids.map(byId).filter(Boolean).map((e) => ({ id: e.id, name: e.name, type: e.type }));
      if (items.length) groups.push({ label: rel.label, items });
    }
    const reverse = {};
    for (const other of world.entities) {
      if (other.id === entity.id) continue;
      for (const rel of relationsFor(other.type)) {
        if (rel.target !== entity.type) continue;
        const ids = (other.links && other.links[rel.key]) || [];
        if (ids.includes(entity.id)) (reverse[rel.reverseLabel] = reverse[rel.reverseLabel] || []).push({ id: other.id, name: other.name, type: other.type });
      }
    }
    for (const label in reverse) groups.push({ label, items: reverse[label] });
    return groups;
  }
  ...
  function deleteEntity(world, entityId) {
    ...
    // Scrub dangling references so no link points at a deleted entity.
    for (const e of world.entities) {
      if (!e.links) continue;
      for (const k in e.links) if (Array.isArray(e.links[k])) e.links[k] = e.links[k].filter((id) => id !== entityId);
    }
  }
  ...
  function cardModel(entity, world) {
    ...
    return {
      ...
      connections: resolveConnections(world, entity).map((g) => ({
        label: g.label,
        items: g.items.map((it) => ({ id: it.id, name: it.name, type: it.type, accent: ACCENTS[it.type] })),
      })),
    };
  }
  ...
  function worldToCSV(world) {
    ...
    for (const e of world.entities) {
      if (e.type === "framework") {
        ...
        // Relationship links, resolved to names (spec roadmap v2).
        for (const rel of relationsFor(e.type)) {
          const ids = (e.links && e.links[rel.key]) || [];
          if (!ids.length) continue;
          const names = ids.map((id) => { const t = world.entities.find((x) => x.id === id); return t ? t.name : null; }).filter(Boolean);
          if (names.length) lines.push([e.type, e.name, tagline, rel.label, names.join("; ")].map(csvEscape).join(","));
        }
      }
    }
    return lines.join("\r\n");
  ...
  function normalizeEntity(e) {
    ...
    if (base.type === "framework") {
      ...
      if (e.links && typeof e.links === "object") {
        for (const k in e.links) if (Array.isArray(e.links[k])) base.links[k] = e.links[k].slice();
      }
    }
    return base;
  }
  ...
  function renderDashboard() {
    ...
    for (const t of TYPE_ORDER) {
      ...
      for (const e of items) {
        ...
        let meta = e.type === "framework" ? (e.summary || "framework") : SCHEMAS[e.type].label;
        if (e.type !== "framework") {
          const connCount = resolveConnections(world, e).reduce((n, g) => n + g.items.length, 0);
          if (connCount) meta += " · 🔗 " + connCount;
        }
  ...
  function openWizard(type, entityId) {
    ...
    if (entityId) {
      ...
      for (const s of steps) {
        if (s.type === "links") draft[s.key] = e.links && Array.isArray(e.links[s.key]) ? e.links[s.key].slice() : [];
        else { const v = e.data[s.key]; draft[s.key] = Array.isArray(v) ? v.slice() : v == null ? "" : v; }
      }
      maxVisited = steps.length - 1;
    } else {
      for (const s of steps) draft[s.key] = s.type === "chips" || s.type === "links" ? [] : "";
    }
  ...
  function renderWizard() {
    ...
    const allSteps = wizardSteps(w.type);
    const step = allSteps[w.stepIndex];
    const isLast = w.stepIndex === allSteps.length - 1;
    ...
    rail.append(el("div", { class: "wiz-rail__sub" }, "Step " + (w.stepIndex + 1) + " of " + allSteps.length));
  ...
  function stepHeadline(type, step) {
    ...
    const map = {
      ...
      settings: "Where do they belong?", characters: "Link the characters involved",
  ...
  function renderField(step, w) {
    ...
    if (step.type === "text") {
      ...
      field.append(renderLinksField(step, w));
    }
  ...
  function renderLinksField(step, w) {
    const world = activeWorld();
    const selected = Array.isArray(w.draft[step.key]) ? w.draft[step.key] : [];
    const options = world.entities.filter((e) => e.type === step.target && e.id !== w.entityId);
    const box = el("div", {});
    if (!options.length) {
      box.append(el("div", { class: "links-empty" },
        "No " + step.target + "s in this world yet. Create one, then come back and link it here."));
      return box;
    }
    box.append(el("div", { class: "links-hint" }, step.single ? "Pick one." : "Pick any that apply."));
    const list = el("div", { class: "choices" });
    options.forEach((opt) => {
      const isSel = selected.includes(opt.id);
      list.append(el("button", {
        class: "choice link-choice" + (isSel ? " is-selected" : ""), type: "button",
        "aria-pressed": isSel ? "true" : "false", style: "--accent:" + ACCENTS[opt.type],
        onClick: () => {
          let arr = Array.isArray(w.draft[step.key]) ? w.draft[step.key].slice() : [];
          if (step.single) arr = isSel ? [] : [opt.id];
          else arr = isSel ? arr.filter((id) => id !== opt.id) : arr.concat(opt.id);
          w.draft[step.key] = arr;
          render();
        },
      },
        el("span", { class: "choice__radio", "aria-hidden": "true" }),
        el("span", {},
          el("span", { class: "choice__label" }, opt.name),
          el("span", { class: "choice__hint" }, " — " + (opt.data && opt.data.tagline ? opt.data.tagline : SCHEMAS[opt.type].label)))));
    });
    box.append(list);
    return box;
  }
  ...
  function currentStep() { return wizardSteps(ui.wizard.type)[ui.wizard.stepIndex]; }
  ...
  function wizardNext() {
    ...
    const steps = wizardSteps(ui.wizard.type);
    if (ui.wizard.stepIndex < steps.length - 1) {
      ui.wizard.stepIndex++;
  ...
  function wizardAdvanceOrCommit() {
    const steps = wizardSteps(ui.wizard.type);
    if (ui.wizard.stepIndex === steps.length - 1) commitEntity();
    else wizardNext();
  }
  ...
  function commitEntity() {
    const w = ui.wizard;
    const schema = SCHEMAS[w.type];
    const steps = wizardSteps(w.type);
    ...
    if (!entity) { entity = { id: uid("ent"), type: w.type, created: Date.now(), data: {}, links: {} }; world.entities.push(entity); }
    const data = {};
    const links = {};
    for (const s of steps) {
      let v = w.draft[s.key];
      if (s.type === "links") { links[s.key] = Array.isArray(v) ? v.slice() : []; continue; }
  ...
  function buildCardDOM(m) {
    ...
    if (m.connections && m.connections.length) {
      const conn = el("div", { class: "card-connections" });
      conn.append(el("div", { class: "card-connections__label" }, "Connections"));
      for (const g of m.connections) {
        conn.append(el("div", { class: "card-row" },
          el("div", { class: "card-row__k" }, g.label),
          el("div", { class: "card-row__v" }, el("div", { class: "card-chips" },
            g.items.map((it) => el("button", {
              class: "conn-chip", type: "button", dataset: { accent: it.type },
              onClick: () => { const wd = activeWorld(); const e = wd && wd.entities.find((x) => x.id === it.id); if (e) openEntity(e); },
            }, it.name))))));
      }
      body.append(conn);
    }
  ...
  function cardStandaloneHTML(m, width) {
    ...
    const connRows = (m.connections || []).map((g) => {
      const chips = g.items.map((it) =>
        '<span style="font-size:11px;font-weight:500;padding:2px 9px;border-radius:999px;margin:0 4px 4px 0;display:inline-block;background:' + mix(it.accent, "#fdfaf3", 0.86) + ";color:" + mix(it.accent, "#2a2521", 0.2) + ";border:1px solid " + mix(it.accent, "#ffffff", 0.55) + ';">' + escapeHtml(it.name) + "</span>").join("");
      return '<div style="display:flex;gap:12px;padding:9px 0;border-bottom:1px solid #eee3d0;"><div style="width:88px;flex:none;font-family:' + mono + ";font-size:10px;text-transform:uppercase;letter-spacing:1px;color:" + accent + ';font-weight:600;padding-top:2px;">' + escapeHtml(g.label) + '</div><div style="flex:1;">' + chips + "</div></div>";
    }).join("");
    const connSection = connRows
      ? '<div style="padding:0 24px 8px;"><div style="font-family:' + mono + ';font-size:10px;letter-spacing:1px;text-transform:uppercase;color:#8d8375;margin:2px 0 2px;">Connections</div>' + connRows + "</div>"
      : "";
    return '' +
  ...
  function seedDemo() {
    ...
    const drowning = normalizeEntity({
    ...
    world.entities.push(kestrel, market, drowning);
    // Relationship layer: wire the world together so the demo shows connections.
    kestrel.links = { settings: [market.id] };
    drowning.links = { characters: [kestrel.id], setting: [market.id] };
  ...
  window.__SF = {
    ...
    go, render, seedDemo,
    // data ops
    createWorld, deleteWorld, deleteEntity, importWorldFromJSON, addImportedWorld,
    serializeWorld, worldToCSV, cardModel, buildFrameworkEntity,
    activeWorld, entityCounts, resolveConnections, RELATIONS,
